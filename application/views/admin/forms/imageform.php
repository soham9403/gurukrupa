...
<div class="form-box">
	<div class="header">
		<div class="form-headeing">
			<h2><?php echo(lang("add_details")); ?></h2><!-- <i class="fa fa-times"></i> -->
		</div>
	</div>
	<div class="content">
		<form action="" id="otherdetailsform" method="POST"  onsubmit="return false;">
			<div class="field-box">
				<label><?php echo(lang('name')); ?></label>
				<input type="text" value="<?php if (isset($form_data)) { echo($form_data->name); } ?>" name="name" placeholder="<?php echo(lang('name')); ?>">
			</div>
			<div class="field-box">
				<label><?php echo(lang('is_main')); ?></label>
				<label class="switch">
				  <input type="checkbox" name="is_main" <?php if (isset($form_data) && $form_data->is_main==1) { echo("checked"); } ?>>
	...
	<div class="footer">
		<button class="primary-theme" onclick="send_data('otherdetailsform','<?php if(isset($brands_page)){echo(get_uri('brands/save'));}else{echo(get_uri('achievements/save'));} ?>')"><?php if (isset($form_data)) { echo(lang('update')); }else{echo(lang('submit'));} ?></button>
	</div>
