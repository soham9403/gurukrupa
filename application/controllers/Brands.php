...
class Brands extends MY_controller {
	...
	public function index()
	{
		...
		$this->template->rander('admin/brand',$view_data);
	}
	public function modalform(){
		...
		$view_data['brands_page'] = 1;
		return $this->load->view('admin/forms/imageform',$view_data);
	}
	public function get_data(){
		$data = $this->Other_details_model->get_brand_list();
		$pass_data = array();
      ...
      if(isset($data->image)){
      	if($data->image!=UNAVAILABLE_IMAGE){
	        if(unlink('files/images/'.$data->image)){   
	           if($this->Other_details_model->delete($data->id) && $this->Product_model->delete_products(["brand_id"=>$data->id])){
